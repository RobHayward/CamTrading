...
\begin{frame}{Information and expectations}
If investors are \emph{informed}, expectations are most important
\pause
\begin{block}{}
Prices will move when new information changes expectations
\end{block}
\pause
\vspace{5mm}
Information and expectations are key
\end{frame}
...
EMH is not binary: true or false
\begin{itemize}[<+-| alert@+>]
\pause
...
\item Off-the-run bonds and core bonds
...
\item Hayek - the market as an information-processsing mechanism
\end{itemize}
\end{frame}
