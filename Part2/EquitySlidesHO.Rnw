...
\begin{center}
\includegraphics[height = 3.0in]{../../Pictures/BGsmall}
\end{center}
\end{frame}

\begin{frame}{Warren Buffet}
\begin{itemize}[<+-| alert@+>]
...
\begin{frame}{Moats}
\centering
\includegraphics[scale=0.45]{../../Pictures/moat}
\end{frame}
...
\begin{frame}{Moats}
	Morningstar use the following criteria to measure moats
\begin{itemize}[<+-| alert@+>]
		\pause
...
\begin{frame}{Valuation}
	Please provide a 10 minute presentation on \textbf{ONE} of the following
\pause
\begin{itemize}[<+-| alert@+>]
\item Criteria for identifying corporate success that can be sustained
\item A sector, industry or firm that will generate outsize returns in future, and why \dots
\item An investment where you believe that the risk is lower than that currently expected
\end{itemize}
\end{frame}
