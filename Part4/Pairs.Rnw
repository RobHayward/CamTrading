...
\end{itemize}
\item Firms may be highly valued because they are strong and will be successful in the future
\item Firms may be low valued because they are weak and will struggle
\end{itemize}
\end{frame}
...
\section{Relative value trading}
\begin{frame}{Relative value}
...
\item Apply a regression and check the residuals - are there different regimes? 
\end{itemize}
\end{frame}
