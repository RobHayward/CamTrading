...
\pause
\begin{itemize}[<+-| alert@+>]
\item There evidence that investment decision-making diverges from the idea of \emph{expected utility theory}
\item There are frictions in the way that financial markets operate
\item These cause \emph{serial correlation} and \emph{reversal} in asset prices
\end{itemize}
\pause
\begin{block}{}
...
plot(da1$Date, da1$NC, type = 'l', ylab = "Net Change Oscillator", xlab = "Date", main = "NCO")
abline(h = 1, col = 'red', lty = 2)
abline(h = -1, col = 'red', lty = 2)
@
\end{frame}

\begin{frame}{Relative Strength Index}
...
\begin{frame}{RSI}
<<rsi, warning=FALSE, message=FALSE, fig.height=4.6, echo=FALSE>>=
library(quantmod)
da1$RSI <- RSI(da1$Close)
dat <- as.ts(da1[,c(2:5)])
# to create one graph on top of the other and extend the margins.
par(mfrow = c(2, 1), mar = c(2, 4, 2, 2))
plotOHLC(dat, origin = "2014-12-25", ylab = "SP500", main = "SP500", 
         xlab = "Relative Strength Index", xaxt = "n") 
plot(da1$Date, da1$RSI, type = 'l', ylab = "RSI", xlab = "Date", main = "NCO")
...
\begin{frame}{You try}
\begin{itemize}[<+-| alert@+>]
\item Go to the US version of Yahoo finance
\item Look at the S\&P 500 (\^GSCP)
\item Try some of these technical tools
\item Buy or sell on Monday?
\end{itemize}
\end{frame}
...
\item \href{https://www.quandl.com/blog/interview-with-a-quant-part-oneyf}{Interview with quant 1}
\item \href{https://www.quandl.com/blog/interview-with-a-quant-part-two}{Interview with quant 2}
\item \href{https://blog.quandl.com/interview-with-a-quant-part-three}{Interview with quant 3}
\end{itemize}
\end{frame}
