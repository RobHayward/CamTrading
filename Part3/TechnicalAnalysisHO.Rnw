...
\begin{itemize}[<+-| alert@+>]
\item Create a quantitative trading rule 
\item Explain the inefficiency that the rule is supposed to capture 
\item Test the trading rule on a real market
\item Assess the use of the trading rule
\end{itemize}
\end{frame}
